...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    if (document) {
      const url = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://bayx.com'}${getLegalDocumentRoute('privacy-policy')}`
  ...
  return {
    ...defaultMetadata,
    title: 'Privacy Policy | BayX',
    description:
      'Learn how BayX collects, uses, and protects your personal information. Read our comprehensive Privacy Policy.',
  }
}
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(
              generateLegalStructuredData(
                document,
                `${process.env.NEXT_PUBLIC_SITE_URL || 'https://bayx.com'}${getLegalDocumentRoute('privacy-policy')}`
              )
            ),
          }}
        />
      ...
      {document ? (
        ...
        <section className="pt-32 pb-14 sm:pt-36 md:pt-42 md:pb-16 lg:pb-[88px] xl:pt-[180px]">
          <div className="main-container text-center">
            <h1 className="text-heading-3 sm:text-heading-2 text-secondary dark:text-accent mb-6">Privacy Policy</h1>
            {error ? (
              <p className="text-tagline-1 text-secondary/70 dark:text-accent/70 mb-8">
                Unable to load the Privacy Policy at this time. Please try again later.
              </p>
            ) : (
              <div className="text-tagline-1 text-secondary/70 dark:text-accent/70 mb-8 space-y-4">
                <p>The Privacy Policy content is currently being prepared by our legal team.</p>
                <p className="text-sm">
